return {
  ...
  dependencies = {
    ...
    --   },
    -- },
    -- nvim-cmp and its dependencies - MUST load before lspconfig uses cmp_nvim_lsp
    {
      'hrsh7th/nvim-cmp',
      dependencies = {
        'L3MON4D3/LuaSnip',
  ...
  config = function()
    ...
    -- Disable trailing space warnings globally
    vim.diagnostic.config {
      ...
      virtual_text = {
        severity = {
          min = vim.diagnostic.severity.WARN, -- Only show WARN and above
        },
      },
      -- Specifically filter out trailing-space warnings
      severity = {
        ['trailing-space'] = vim.diagnostic.severity.INFO, -- Downgrade to INFO (or HINT to hide)
      },
    }

    -- Filter specific diagnostic messages by code
    local function filter_diagnostics(_, result, ctx, config)
      ...
      while idx <= #result.diagnostics do
        ...
        -- Add the diagnostic codes you want to ignore here
        -- Common codes to filter:
        -- 536871362 = TODO comment marker
        -- 80001 = CommonJS module message
        -- Add more codes as you see them in your diagnostics
        if entry.code == 536871362 or entry.code == 80001 then
